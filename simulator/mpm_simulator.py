...
class MPMSimulator:
    ...
    def __init__(self, dtype, dt, frame_dt, particle_layout, dx, inv_dx, n_particles, args, gravity=[0, -9.8, 0], material=elasticity, cuda_chunk_size=400, **kargs):
        ...
        self.args = args
        self.non_newtonian = 0
        if self.material == MPMSimulator.von_mises:
            if getattr(self.args, 'kappa', None) and getattr(self.args, 'mu', None):
                self.non_newtonian = 1
                print('material: non_newtonian')
        ...
        self.damping_coeff = getattr(self.args, 'damping', 0.0)
        print(f"damping coeff: {self.damping_coeff}")

        # material
        self.p_vol = ti.field(self.dtype, needs_grad=True)
    ...
    def project_F(self, s: ti.i32):
        for p in range(self.n_particles[None]):
            ...
            if ti.static(self.material == self.viscous_fluid):
                ...
                if self.non_newtonian == 0:
                    epsilon = ti.log(sig)
                    epsilon_hat = epsilon - (epsilon.sum() / self.dim)
                    epsilon_hat_norm = self.norm(epsilon_hat)
                    y = epsilon_hat_norm - self.yield_stress[None] / (2.0 * self.mu[p])
                    if y > 0:
                        H = epsilon - y * epsilon_hat / epsilon_hat_norm
                        sig_out = ti.exp(H)
                        self.F[p, s+1] = self.U[p] @ self.make_matrix_from_diag(sig_out) @ self.V[p].transpose()
                    else:
                        sig_out = sig
    ...
    def p2g(self, s: ti.i32):
        ...
        for p in range(self.n_particles[None]):
            ...
            # print('mass {}'.format(self.p_mass[p]))
            for i in ti.static(range(3)):
                for j in ti.static(range(3)):
                    for k in ti.static(range(3)):
                        ...
                        #     print(idx[0],idx[1],idx[2],self.grid_v_in[base + offset] )
            ...
            for i in ti.static(range(3)):
                for j in ti.static(range(3)):
                    for k in ti.static(range(3)):
                        ...
                        # if base(0) + i < 0 or base(1) + j < 0 or base(2) + k < 0:
                        #     print(g_v)
                        weight = w[i][0] * w[j][1] * w[k][2]
                        new_v += weight * g_v
    ...
    def check_cfl(self, s: ti.i32):
        for p in range(self.n_particles[None]):
            ...
            if ti.abs(self.v[p, s]).max() * self.dt[None] > self.dx[None]:
                self.cfl_satisfy[None] = 0
                # print('ERROR:                      ', ti.abs(self.v[p, s]).max() * self.dt[None] , ' > ', self.dx[None])
